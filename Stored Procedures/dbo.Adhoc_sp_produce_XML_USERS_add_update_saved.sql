...
CREATE PROC [dbo].[Adhoc_sp_produce_XML_USERS_add_update_saved] ( @submit BIT=0 )
AS
...
/*
	>>>> See list of new users to uplood to DM:
...
WITH new_and_updated_current_employees AS (
	...
	--select *  FROM DM_Shadow_Staging.dbo._UPLOAD_DM_USERS DMU
...
reentry_employees AS (
	...
	WHERE username IN (SELECT USERNAME FROM dbo._DM_USERS WHERE USERNAME is not NULL AND Enabled_Indicator=0)
			AND username not in  (SELECT username FROM DM_Shadow_Staging.dbo._DM_ADMIN WHERE KEEP_ACTIVE='No' AND AC_YEAR='2017-2018' AND username is not NULL)
	--  SPECIAL UPLOAD NEW USERS CERTAIN USERS
	--  WHERE USERNAME IN ('holland6','kdedward','mhl')
	--  WHERE USERNAME IN ('holland6','m-leroy')
)
...
FROM (
	...
	UNION
	-- Assign re-entry users to their department
	
	SELECT USERNAME,3,'PUT','/login/service/v4/UserSchema/USERNAME:'+ USERNAME + '/INDIVIDUAL-ACTIVITIES-Business' as url,
		CAST((SELECT 
			--CAST(YEAR(GETDATE())AS VARCHAR)+'-'+CAST(YEAR(GETDATE())+1 AS VARCHAR)AC_YEAR,
			'2018-2019' AC_YEAR	,
			(
