...
CREATE PROCEDURE [dbo].[_Decommissioned_shadow_DEG_COMMITTEE_v3] (@webservices_requests_id INT,@xml XML,@userid BIGINT=NULL,@resync BIT=NULL) 
AS 
			...
			BEGIN
				...
				IF @userid is not null
					BEGIN
						...
						-- Update records of @userid at relational tables _DM_DEG_COMMITTEE_MEMBER in DM_Shadow_Staging and DM_Shadow_Production databases		    
						SET @fields = 'id,itemid,lastModified,Create_Datetime,Download_Datetime' +
										',FACULTY_NAME,FNAME,MNAME,LNAME,[ROLE],sequence'
						EXEC dbo.shadow_screen_data2 @webservices_requests_id=@webservices_requests_id
							,@table='_DM_DEG_COMMITTEE_MEMBER'
							,@cols=@fields
							,@userid=@userid
					...
					BEGIN
						...
						IF @new_record_main_count >= @current_record_main_count
							...
							BEGIN
								...
								-- Truncate and Insert into Relational tables _DM_DEG_COMMITTEE_MEMBER in DM_Shadow_Staging and DM_Shadow_Production databases							
								SET @fields = 'id,itemid,lastModified,Create_Datetime,Download_Datetime' +
										',FACULTY_NAME,FNAME,MNAME,LNAME,[ROLE],sequence'
								EXEC dbo.shadow_screen_data2 @webservices_requests_id=@webservices_requests_id
									,@table='_DM_DEG_COMMITTEE_MEMBER'
