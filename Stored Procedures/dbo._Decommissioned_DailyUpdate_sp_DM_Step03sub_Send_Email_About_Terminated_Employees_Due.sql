...
CREATE  Procedure [dbo].[_Decommissioned_DailyUpdate_sp_DM_Step03sub_Send_Email_About_Terminated_Employees_Due]
as
		...
		WHERE	bus_person_indicator = 1  
			...
			(
				SELECT     edw_pers_id
				FROM         dbo._FSDB_EDW_Current_Employees
		...
		SET @header = @header + '>>>  Procedure: DM_Shadow_Staging.dbo.DailyUpdate_sp_Send_Email_About_Terminated_Employees_Due <BR><BR>' + @CRLF
		SET @footer = '<BR></HTML>'
		...
		BEGIN
			...
			BEGIN
				...
				IF @count > 50
					SET @email_subject = 'Employee Termination limit exceeded - LIST part ' + cast(@j as varchar)
				ELSE
					SET @email_subject = 'Employee Termination limit exceeded - LIST'
				...
				--EXEC dbo.DailyUpdate_sp_Send_Email 'research@business.illinois.edu','scasteel@illinois.edu','research@business.illinois.edu',@email_subject, @email_body1
