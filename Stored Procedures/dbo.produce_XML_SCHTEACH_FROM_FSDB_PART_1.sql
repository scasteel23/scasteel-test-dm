...
CREATE PROC [dbo].[produce_XML_SCHTEACH_FROM_FSDB_PART_1] ( @submit BIT=0 )
AS
...
BEGIN
	...
	FROM (
		...
		CAST((SELECT username "@username"
					  ...
					  ,DELIVERY_MODE 
					  ,CRN
					  --,(select DEGREE_PROGRAM1 as [text()] for xml path('DEGREE_PROGRAM'), type)
					  --,(select DEGREE_PROGRAM2 as [text()] for xml path('DEGREE_PROGRAM'), type)
					  --,(select DEGREE_PROGRAM3 as [text()] for xml path('DEGREE_PROGRAM'), type)
