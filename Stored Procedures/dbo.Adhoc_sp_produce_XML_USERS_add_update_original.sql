...
CREATE PROC [dbo].[Adhoc_sp_produce_XML_USERS_add_update_original] ( @submit BIT=0 )
AS
...
reentry_employees AS (
	-- reentry employees are those who had been employees, quit, but returned to the college. Welcome back!
	SELECT  USERNAME,UIN, FACSTAFFID, EDWPERSID 
			,DEP 
			,FIRST_NAME,MIDDLE_NAME,LAST_NAME
			,Record_Status
