...
-- NS 3/25/2019 Run special for those who have not titles yet on DM
-- NS 1/2/2019 Tested, works
-- NS 12/21/2018
--		Copied from ADMIN_DEP for ADMIN_TITLE
--		6 minutes for 600 records
-- NS 11/7/2018
...
-- NS 9/18/2018: 
CREATE PROC [dbo].[produce_XML_ADMIN_Update_ADMIN_TITLE] ( @submit BIT=0 )
AS
...
/*
	...
	There is a daily upload of new users to DM_Shadow_Staging.dbo._DM_USERS table
	 ...
	 EXEC dbo.[produce_XML_ADMIN_Update_ADMIN_TITLE] @submit = 0

	 Manual run to upload USERS FROM FSDB  -- 17 minutes
	 EXEC dbo.[produce_XML_ADMIN_Update_ADMIN_TITLE] @submit = 1
...
/login/service/v4/UserSchema/USERNAME:brownjr/INDIVIDUAL-ACTIVITIES-Business
<INDIVIDUAL-ACTIVITIES-Business><ADMIN><AC_YEAR>2018-2019</AC_YEAR><ADMIN_TITLE>
<TITLE>Josef and Margot Lakonishok Professor of Business</TITLE><TITLE_CURRENT>Yes</TITLE_CURRENT>
</ADMIN_TITLE><ADMIN_TITLE><TITLE>Dean</TITLE><TITLE_CURRENT>Yes</TITLE_CURRENT></ADMIN_TITLE></ADMIN>
</INDIVIDUAL-ACTIVITIES-Business>

<INDIVIDUAL-ACTIVITIES-Business><ADMIN><AC_YEAR>2018-2019</AC_YEAR>
<ADMIN_TITLE><TITLE>Josef and Margot Lakonishok Professor of Business</TITLE><TITLE_CURRENT>Yes</TITLE_CURRENT></ADMIN_TITLE>
<ADMIN_TITLE><TITLE>Dean</TITLE><TITLE_CURRENT>Yes</TITLE_CURRENT></ADMIN_TITLE></ADMIN></INDIVIDUAL-ACTIVITIES-Business>

<INDIVIDUAL-ACTIVITIES-Business>
<ADMIN><AC_YEAR>2018-2019</AC_YEAR><ADMIN_TITLE><TITLE>Dean</TITLE><TITLE_CURRENT>Yes</TITLE_CURRENT></ADMIN_TITLE>
<ADMIN_TITLE><TITLE>Josef and Margot Lakonishok Professor of Business</TITLE><TITLE_CURRENT>Yes</TITLE_CURRENT></ADMIN_TITLE></ADMIN>
</INDIVIDUAL-ACTIVITIES-Business>
...
WITH existdept_current_employees AS (
	...
	SELECT  DISTINCT DMU.USERNAME,DMU.UIN, CAST(DMU.FACSTAFFID as varchar) as FACSTAFFID, CAST(DMU.EDWPERSID as varchar) as EDWPERSID 
		...
		WHERE  username in (SELECT [USERNAME]FROM [DM_Shadow_Staging].[dbo].[_DM_ADMIN] where ac_year='2018-2019')
				...
				-- NS 3/25/2019 
				AND DMU.username NOT IN (SELECT USERNAME FROM dbo._DM_ADMIN_TITLE where ac_year='2018-2019' AND USERNAME IS NOT NULL )
				AND DMU.FacstaffID IN (SELECT Facstaff_ID FROM Faculty_Staff_Holder.dbo.Facstaff_Titles)
...
title1 as (
		SELECT DMU.USERNAME,CAST(DMU.FACSTAFFID as varchar) as FACSTAFFID, CAST(DMU.EDWPERSID as varchar) as EDWPERSID 
			,FST.Title_Name as TITLE
			,CASE WHEN FST.Active_Indicator=1 THEN 'Yes' ELSE 'No' END as TITLE_CURRENT
			,FST.Title_Sequence as SEQ
		FROM DM_Shadow_Staging.dbo._DM_USERS DMU 
				INNER JOIN Faculty_Staff_Holder.dbo.Facstaff_TITLES FST				
				ON DMU.FacstaffID = FST.Facstaff_ID
					AND FST.Active_Indicator=1
...
FROM (
	SELECT USERNAME,3 as o,'PUT' method
		...
		,(
			SELECT TITLE, TITLE_CURRENT			
			FROM title1 d WHERE d.username = c.username 	ORDER BY USERNAME, SEQ ASC
			FOR XML PATH('ADMIN_TITLE'),TYPE
		 ) 
...
-- >>>>>>>>>>>>>>>>>  2. "POST" to create new ADMIN_TITLE and ADMIN
--				Add to ADMIN (POST): This DM add relies on the most recent _DM_USERS
--				Any record on _DM_USERS that not in _DM_ADMINS will be added ADMIN
...
WITH nodept_current_employees AS (
		...
		FROM DM_Shadow_Staging.dbo._DM_USERS DMU
				INNER JOIN dbo.FSDB_Facstaff_Basic FB
				ON DMU.FacstaffID = FB.Facstaff_ID
		WHERE  username not in (SELECT [USERNAME]FROM [DM_Shadow_Staging].[dbo].[_DM_ADMIN])
...
SELECT USERNAME,3 as o,'POST' method
	...
	,(
		SELECT TITLE, TITLE_CURRENT 			
		FROM title2 d WHERE d.username = c.username
		FOR XML PATH('ADMIN_TITLE'),TYPE
		) 
