...
-- NS 4/22/2019: update names to newly added BANNER_FNAME, BANNER_MNAME, BANNER_LNAME
-- NS 10/26/2017: Took off FACSTAFFID and EDWPERSID tags, must be uploaded at USERS screen not in PCI screen
-- NS 8/22/2017: revised from [produce_XML_PCI_new]
CREATE PROC [dbo].[produce_XML_PCI_current] ( @submit BIT=0 )
AS
...
BEGIN
	...
	SELECT  username
		...
		WHERE USERNAME IN (SELECT USERNAME FROM _DM_PCI WHERE USERNAME is not NULL)		
	...
	-- Fill in their Personal Information
	 ...
	 SELECT username,3 o,'POST' method,'/login/service/v4/SchemaData/INDIVIDUAL-ACTIVITIES-Business' url,
		CAST((
			...
			-- NS 4/22/2019: update naes to newly added BANNER_FNAME, BANNER_MNAME, BANNER_LNAME
