...
CREATE PROCEDURE [dbo].[_Decommissioned_shadow_PRESENT_v1] (@xml XML,@userid BIGINT=NULL,@resync BIT=NULL) 
AS 
...
/*
	...
	*/
	...
	IF @userid IS NULL AND (SELECT COUNT(*) FROM #_DM_PRESENT) < 2 RAISERROR('No Data',18,1) -- just to make sure we have some records, change the threshold to 10 or more on production
	...
	ELSE BEGIN
		...
		ELSE BEGIN
			...
			SET @fields2 = 'userName,lastModified' +
							...
							',LNAME,ROLE'
			EXEC dbo.shadow_screen_data '_DM_PRESENT_AUTH','_DM_PRESENT',@fields2,@userid,@resync;
