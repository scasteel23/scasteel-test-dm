...
/*
	Manual run to shadow individual PROFILE screen
	EXEC dbo.webservices_initiate @screen='PROFILE'
	EXEC dbo.webservices_run_DTSX

	EXEC dbo.webservices_initiate @username='brownjr'		-- 18 seconds
	EXEC dbo.webservices_run_DTSX
*/

CREATE PROCEDURE [dbo].[shadow_PROFILE] (@webservices_requests_id INT, @xml XML,@userid BIGINT=NULL,@resync BIT=NULL) 
AS 
	...
	ELSE BEGIN
		...
		ELSE	BEGIN 
			...
			IF @userid is not null
					BEGIN
						-- Update records of @userid at Main tables _DM_PROFILE in DM_Shadow_Staging and DM_Shadow_Production databases
						SET @fields = 'userid,id,username,lastModified,Create_Datetime,Download_DateTime' +
						...
						-- Update records of @userid at relational tables _DM_PROFILE_LANGUAGES in DM_Shadow_Staging and DM_Shadow_Production databases		    					
						SET @fields = 'id,itemid,USERNAME,lastModified,Create_Datetime,Download_Datetime' +
												',FLUENCY,LANGUAGE,LANGUAGE_OTHER '
						EXEC dbo.shadow_screen_data2 @webservices_requests_id=@webservices_requests_id
							,@table='_DM_PROFILE_LANGUAGES'
							,@cols=@fields
							,@userid=@userid
				...
				BEGIN
					...
					BEGIN TRY
						...
						IF @new_record_main_count >= @current_record_main_count
							...
							BEGIN
								-- Update Main tables _DM_PROFILE in DM_Shadow_Staging and DM_Shadow_Production databases
								SET @fields = 'userid,id,username,lastModified,Create_Datetime,Download_DateTime' +
										',BIO,PROF_INTERESTS,OTHER_INTERESTS,TEACHING_INTERESTS,RESEARCH_INTERESTS,SPECIALIZATION'
								EXEC dbo.shadow_screen_data2 @webservices_requests_id=@webservices_requests_id
									,@table='_DM_PROFILE'
								...
								-- Truncate and Insert into Main tables _DM_PROFILE_LANGUAGES in DM_Shadow_Staging and DM_Shadow_Production databases											
								SET @fields = 'id,itemid,USERNAME,lastModified,Create_Datetime,Download_Datetime' +
												',FLUENCY,LANGUAGE,LANGUAGE_OTHER '
								EXEC dbo.shadow_screen_data2 @webservices_requests_id=@webservices_requests_id
									,@table='_DM_PROFILE_LANGUAGES'
