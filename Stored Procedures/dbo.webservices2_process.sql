...
-- NS 4/3/2019
--  This procedure is called from [webservices2_RUN]
--  Need to pass webservices_requests.id to each shadow_* SP in order to save the error into webservices_requests.SP_Error
--		i.e. EXEC dbo.shadow_PCI @id,@response,@resync=0
CREATE PROCEDURE [dbo].[webservices2_process](@id INT, @response TEXT, @responseCode VARCHAR(20)) 
AS 
...
BEGIN
	...
	SET @prefix = @prefix + ' - webservices2_process started '
	...
	SET @responseXML = 	CAST(REPLACE(CAST(@response AS NVARCHAR(MAX)),'utf-8','utf-16') AS XML)
	...
	IF (SELECT method FROM dbo.webservices_requests WHERE [id]=@id) = 'GET' BEGIN
		...
		ELSE IF @urlDetail='PCI' EXEC dbo.shadow_PCI @id,@responseXML,@resync=0				-- works						-- get all PCI (personal) screen data
