...
SET ANSI_NULLS ON
GO
-- NS 8/21/2020 add first hire, first work, cur_hire
-- STC 6/19/19 - Get Dept from DM instead of FSB
-- NS 5/22/2019

CREATE PROC [dbo].[DSQUERY_sp_All_GiesBusiness_Employees]
AS
SELECT distinct [UIN]
      ,[EDW_PERS_ID]
	  ,[Network_ID]
...
-- STC need to do: temporary fix until FSDB_Facstaff_Basic is updated daily from DM data
   ...
   --   ,getdate() as First_Hire_Date
	  --,getdate() as First_Work_Date
	  --,getdate() as Current_Hire_Date

	  ,cast(NULL as datetime) as First_Hire_Date
	  ,cast(NULL as datetime) as First_Work_Date
	  ,cast(NULL as datetime) as Current_Hire_Date
...
-- STC temporary fix until FSDB_Facstaff_Basic is updated daily from DM data
  ...
  UPDATE #FB
  SET First_Hire_Date = rawdata.FIRST_HIRE_DT
	, First_Work_Date = rawdata.FIRST_WORK_DT
	, Current_Hire_Date = rawdata.CUR_HIRE_DT
  --FROM [DM_Shadow_Staging].[dbo].[_DM_BANNER] rawdata
  --WHERE  #fb.EDW_PERS_ID = rawdata.EDWPERSID
  FROM 	[DM_Shadow_Staging].[dbo].[FSDB_EDW_Current_Employees] rawdata
  WHERE rawdata.Create_Datetime>='2019-08-21 01:40:35.460'
		AND #fb.EDW_PERS_ID = rawdata.EDW_PERS_ID
		AND rawdata.FIRST_HIRE_DT IS NOT NULL
...
-- STC need to do: temporary fix until FSDB_Facstaff_Basic is updated daily from DM data
	  ...
	  ,Convert(varchar,First_Hire_Date,101) as First_Hire_Date
	  ,Convert(varchar,Current_Hire_Date,101) as Current_Hire_Date
	  ,Convert(varchar,First_Work_Date,101) as First_Work_Date

     FROM #FB
	 ORDER BY [EMPEE_GROUP_CD] ASC, PERS_Lname, PERS_FNAME
