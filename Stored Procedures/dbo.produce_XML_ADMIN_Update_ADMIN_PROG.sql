...
-- NS 1/2/2019: Tested, it works
-- NS 12/21/2018
--		Copied from ADMIN_DEP for ADMIN_PROG
...
-- NS 9/18/2018: 
CREATE PROC [dbo].[produce_XML_ADMIN_Update_ADMIN_PROG] ( @submit BIT=0 )
AS
...
/*
	...
	There is a daily upload of new users to DM_Shadow_Staging.dbo._DM_USERS table
	 ...
	 EXEC dbo.[produce_XML_ADMIN_Update_ADMIN_PROG] @submit = 1
...
<ADMIN>
<AC_YEAR>2018-2019</AC_YEAR>
	<ADMIN_PROG><DEP>iMBA</DEP></ADMIN_PROG>
</ADMIN>
</INDIVIDUAL-ACTIVITIES-Business>
...
-- PUT to update existing ADMIN_PROG
-- POST to create new ADMIN_PROG -- based on _DM_USERS as users to compare with _DM_ADMINS
...
-- >>>>>>>>>>>>>>>>>> 1. "PUT" to update existing ADMIN_PROG, ADMIN_PROG, ADMIN_NPRESP, ADMIN_EMPTYPE, ADMIN_EMPGROUP, ADMIN_TITLE
--			Update to ADMIN (PUT): This DM update relies on the most recent _DM_ADMIN and Facstaff_Basic
--			Any record in _DM_ADMIN will get updates if the user exist in _DM_BANNER
...
WITH existdept_current_employees AS (
	...
	SELECT  DISTINCT DMU.USERNAME,DMU.UIN, CAST(DMU.FACSTAFFID as varchar) as FACSTAFFID, CAST(DMU.EDWPERSID as varchar) as EDWPERSID 
		FROM DM_Shadow_Staging.dbo._DM_USERS DMU
				INNER JOIN Faculty_Staff_Holder.dbo.Facstaff_Basic FB
				ON DMU.FacstaffID = FB.Facstaff_ID
		WHERE  username in (SELECT [USERNAME]FROM [DM_Shadow_Staging].[dbo].[_DM_ADMIN])
...
prog1 as (
		SELECT DMU.USERNAME,CAST(DMU.FACSTAFFID as varchar) as FACSTAFFID, CAST(DMU.EDWPERSID as varchar) as EDWPERSID 
			,SC.Subunit_ID 
			,sc.DM_ADMIN_PROG as ADMIN_PROG       
		FROM DM_Shadow_Staging.dbo._DM_USERS DMU 
				INNER JOIN Faculty_Staff_Holder.dbo.Facstaff_Subunits FSJD
					INNER JOIN Faculty_Staff_Holder.dbo.Subunit_Codes SC
					ON FSJD.Subunit_ID = SC.Subunit_ID
				ON DMU.FacstaffID = FSJD.Facstaff_ID
					AND FSJD.Active_Indicator=1
)
...
FROM (
	SELECT USERNAME,3 as o,'PUT' method
		...
		,(
			SELECT ADMIN_PROG as PROG				
			FROM prog1 d WHERE d.username = c.username
			FOR XML PATH('ADMIN_PROG'),TYPE
		 ) 
...
-- >>>>>>>>>>>>>>>>>  2. "POST" to create new ADMIN_PROG and ADMIN
--				Add to ADMIN (POST): This DM add relies on the most recent _DM_USERS
--				Any record on _DM_USERS that not in _DM_ADMINS will be added ADMIN
...
SELECT USERNAME,3 as o,'POST' method
	...
	,(
		SELECT ADMIN_PROG as PROG				
		FROM prog2 d WHERE d.username = c.username
		FOR XML PATH('ADMIN_PROG'),TYPE
		) 
...
FROM (
	SELECT USERNAME,3 as o,'PUT' method,'login/service/v4/UserSchema/USERNAME:'+ USERNAME + '/INDIVIDUAL-ACTIVITIES-Business' as url,
		...
		/* '<INDIVIDUAL-ACTIVITIES-University><ADMIN>'+
			'<AC_YEAR>'+CAST(YEAR(GETDATE())AS VARCHAR)+'-'+CAST(YEAR(GETDATE())+1 AS VARCHAR)+'</AC_YEAR><ADMIN_PROG>'+
