...
-- NS 3/22/2017: V2
--			Treated resynching _DM_PRESENT, _DM_CONGRANT, _DM_INTELLCONT, _DM_PRESENT_AUTH, _DM_CONGRANT_INVEST, and _DM_INTELLCONT_AUTH
--			tables differently since their primary keys are different with other tables
--			_DM_PRESENT, _DM_CONGRANT, _DM_INTELLCONT: {id, userid} 
--			_DM_PRESENT_AUTH, _DM_CONGRANT_INVEST, and _DM_INTELLCONT_AUTH {itemid, userid}
--			the others {id}
--		
...
CREATE PROCEDURE [dbo].[_Decommissioned_shadow_screen_data_v1] (@table VARCHAR(MAX)
	...
	ELSE 
		...
		ELSE 
		    BEGIN
				...
				-- NS 3/22/2017: treat _DM_PRESENT, _DM_CONGRANT, _DM_INTELLCONT, _DM_PRESENT_AUTH, _DM_CONGRANT_INVEST, and _DM_INTELLCONT_AUTH
				--		tables differently since their primary keys are different
				DECLARE @sql AS VARCHAR(MAX) 
				IF @table = '_DM_PRESENT' OR @table = '_DM_CONGRANT' OR @table = '_DM_INTELLCONT'
						OR @table = '_DM_PRESENT_AUTH' OR @table = '_DM_CONGRANT_INVEST' OR @table = '_DM_INTELLCONT_AUTH'
					BEGIN
						-- Primary Keys are userid and id fields
						SET @sql = '
						-- Pass Variables
						...
						IF @resync=1
							MERGE INTO dbo.'+@table+' a USING DM_Shadow_Production.dbo.'+@table+' b ON (a.'+@id+'=b.'+@id+' AND a.userid=b.userid)
							WHEN MATCHED AND (
								'+@colsChangeTest + 
							') THEN UPDATE SET
								'+@colsSet+'
						...
						MERGE INTO dbo.'+@table+' a USING #'+@table+' b ON (a.'+@id+'=b.'+@id+' AND a.userid=b.userid)
						WHEN MATCHED AND (
								'+@colsChangeTest+'
							) THEN UPDATE SET
								'+@colsMergeSet+' 
						...
						BEGIN TRY
							...
							JOIN #changes'+@table+' b ON a.'+@id+'=b.'+@id+' AND a.userid=b.userid
							WHERE b.action=''U'';
			
							'+@sql_debug+'''Production DELETE''
							DELETE FROM DM_Shadow_Production.dbo.'+@table+' WHERE EXISTS (SELECT '+@id+', userid FROM #changes'+@table+' WHERE action=''D'')
