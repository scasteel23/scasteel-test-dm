...
-- NS 8/24/2020
--		Fixed by extending size the following fields, all was 30 chars except preferred fname of 15 chars!!
--			Last_Name varchar(60) NOT NULL,
--			PERS_MNAME varchar(60) NULL,
--			Middle_Name varchar(60) NULL,
--			First_Name varchar(60) NULL,
--			PERS_PREFERRED_FNAME varchar(60) NULL,
-- STC 12/19/19 - Disable Rank + Tenure updates
-- NS 5/8/2019: 
--		Update ADMIN_DEP, ADMIN_EMPTYPE, ADMIN_EMPGROUP when resp. data in DM is empty
-- NS 4/25/2019

CREATE PROC [dbo].[produce_XML_ADMIN_PCI_USER_Update] ( @submit BIT=0 )
AS
...
/*
	 ...
	 2) UPDATE ADMIN screen related to AC_YEAR='2018-2019'; ADMIN, ADMIN_DEP, ADMIN_EMPTYPE, ADMIN_EMPGROUP
			Track changes in RANK,FTE,FTE_EXTERN,Tenure	
		UPDATE ADMIN_EMPGROUP screen
			Track additional EMPGROUP
		UPDATE ADMIN_EMPTYPE screen
			Track additional EMPTYPE
	 3) UPDATE PCI screen
		  Track changes in GENDER,BANNER_LNAME,BANNER_MNAME,BANNER_FNAME,ETHNICITY,CITIZEN,DOB_START

	  
	 Test
	 EXEC dbo.produce_XML_ADMIN_PCI_USER_Update @submit = 0
...
PRINT 'start produce_XML_ADMIN_PCI_USER_Update';
...
FROM DM_Shadow_Staging.dbo.FSDB_Facstaff_Basic FB 
		...
		ON FB.EDW_PERS_ID = DMA.EDWPERSID AND DMA.AC_YEAR=@AC_YEAR AND FB.Facstaff_ID = DMA.FACSTAFFID;
...
-- Set to NULL when fsdb fte is 0 but the dm fte is NULL
UPDATE @TempTemp
SET College_Sum_FTE = dma.FTE
FROM @TempTemp T INNER JOIN DM_Shadow_Staging.dbo._DM_ADMIN dma
			ON T.username = dma.username AND DMA.AC_YEAR=@AC_YEAR
...
WHERE username in 
	( SELECT T.username
	  ...
	  WHERE --(T.[RANK] <> '' AND T.[RANK]<> dma.[RANK])
			--OR 
			(T.College_Sum_FTE is not null AND T.College_Sum_FTE <> dma.FTE)
...
-- >>>>>>>>>>>>>>>>>  2) UPDATE ADMIN screen related to AC_YEAR='2018-2019'; 
--				ADMIN, ADMIN_DEP, ADMIN_EMPTYPE, ADMIN_EMPGROUP
--				For any record on _DM_USERS that are in _DM_ADMIN
...
FROM (
	SELECT USERNAME,3 as o,'PUT' method
		...
		,CAST((SELECT @AC_YEAR AC_YEAR		
		,(
			SELECT DM_Department_Name as DEP, 'Yes' as SHOW_DIRECTORY				
...
FROM (
	SELECT USERNAME,3 as o,'PUT' method
		...
		,CAST(
		 (SELECT @AC_YEAR AC_YEAR		
		,(
			--EMPGROUP was implemented as Checkboxes, not DSA, no need to hav XM PATH, instead add the alias EMPGROUP to be a <EMPGROUP> tag later
			SELECT  EMPGROUP 									
...
--SELECT * FROM @TempTemp order by username
