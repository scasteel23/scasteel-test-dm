...
CREATE PROCEDURE [dbo].[_Decommissioned_shadow_PRESENT_v2] (@webservices_requests_id INT,@xml XML,@userid BIGINT=NULL,@resync BIT=NULL) 
AS 
	...
	ELSE 
		...
		ELSE 
			...
			IF @new_record_main_count >= @current_record_main_count
				...
				BEGIN
					-- we can copy FROM (#_DM_PRESENT, and #_DM_PRESENT_AUTH) TO (_DM_PRESENT, and _DM_PRESENT_AUTH)

					-- >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
					-- @DM_Shadow_Staging database
					-- >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
					TRUNCATE TABLE dbo._DM_PRESENT

					INSERT INTO dbo._DM_PRESENT
						(
					...
					--UPDATE dbo._DM_PRESENT 
					--	SET FACSTAFFID = U.FACSTAFFID, EDWPERSID=U.EDWPERSID 
					--FROM dbo._DM_USERS U, dbo._DM_PRESENT F
					--WHERE U.userid = F.userid AND ( F.FACSTAFFID is NULL OR F.EDWPERSID IS NULL )			

					TRUNCATE TABLE dbo._DM_PRESENT_AUTH
					INSERT INTO dbo._DM_PRESENT_AUTH
						(
					...
					INSERT INTO DM_Shadow_Production.dbo._DM_PRESENT
						(
							 [id], lastModified, Create_Datetime, Download_Datetime
							,TITLE,CLASSIFICATION,NAME,STATUS,REFEREED,CITY,STATE,COUNTRY                       
							,MEETING_TYPE,SCOPE_LOCALE,ORG,PUBPROCEED,[DESC]                                   
					...
					SELECT  [id], lastModified, Create_Datetime, Download_Datetime
							...
							,DOI,SSRN_ID,DTM_DATE,DTY_DATE
							 --,USER_REFERENCE_CREATOR				
					FROM dbo._DM_PRESENT
												
					--UPDATE DM_Shadow_Production.dbo._DM_PRESENT
					--SET FACSTAFFID = U.FACSTAFFID, EDWPERSID=U.EDWPERSID 
					--FROM DM_Shadow_Production.dbo._DM_USERS U, DM_Shadow_Production.dbo._DM_PRESENT F
					--WHERE U.userid = F.userid AND ( F.FACSTAFFID is NULL OR F.EDWPERSID IS NULL )

					TRUNCATE TABLE DM_Shadow_Production.dbo._DM_PRESENT_AUTH
					INSERT INTO DM_Shadow_Production.dbo._DM_PRESENT_AUTH
						(
							[id], itemid, lastModified, Create_Datetime
							,FACULTY_NAME,FNAME,MNAME
							,LNAME,[ROLE],sequence
