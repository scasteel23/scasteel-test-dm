...
--		Newly made for update/add ADMIN_EMPGROUP that was implemented as checkboxes not a DSA
...
-- NS 9/18/2018: 
CREATE PROC [dbo].[produce_XML_ADMIN_Update_ADMIN_EMPGROUP] ( @submit BIT=0 )
AS
...
/*
	...
	There is a daily upload of new users to DM_Shadow_Staging.dbo._DM_USERS table
	 ...
	 EXEC dbo.[produce_XML_ADMIN_Update_ADMIN_EMPGROUP] @submit = 0

	 Manual run to upload USERS FROM FSDB  -- 17 minutes
	 EXEC dbo.[produce_XML_ADMIN_Update_ADMIN_EMPGROUP] @submit = 1
...
-- PUT to update existing ADMIN_EMPGROUP
-- POST to create new ADMIN_EMPGROUP -- based on _DM_USERS as users to compare with _DM_ADMINS
...
-- >>>>>>>>>>>>>>>>>> 1. "PUT" to update existing ADMIN_EMPGROUP, ADMIN_PROG, ADMIN_NPRESP, ADMIN_EMPTYPE, ADMIN_EMPGROUP, ADMIN_TITLE
--			Update to ADMIN (PUT): This DM update relies on the most recent _DM_ADMIN and Facstaff_Basic
--			Any record in _DM_ADMIN will get updates if the user exist in _DM_BANNER
...
---- >>>>>>>>>>>>>>>>>  2. "POST" to create new ADMIN_EMPGROUP and ADMIN
----				Add to ADMIN (POST): This DM add relies on the most recent _DM_USERS
----				Any record on _DM_USERS that not in _DM_ADMINS will be added ADMIN
...
---- POST to create new ADMIN_EMPGROUP
--INSERT INTO #Updates (m,u,post,username,o,r)
--SELECT method m,url u,xml post, USERNAME,o,ROW_NUMBER()OVER(ORDER BY USERNAME,o,url)r
--FROM (
...
--		--EMPGROUP was implemented as Checkboxes, not DSA, no need to hav XM PATH, instead add the alias EMPTYPE to be a <EMPGROUP> tag later
--		SELECT  EMPGROUP
--		FROM empgroup2 d WHERE d.username = c.username
--		FOR XML PATH(''),TYPE
--		) 
