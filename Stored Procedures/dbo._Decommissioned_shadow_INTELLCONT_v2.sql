...
CREATE PROCEDURE [dbo].[_Decommissioned_shadow_INTELLCONT_v2] (@webservices_requests_id INT, @xml XML,@userid BIGINT=NULL,@resync BIT=NULL) 
AS 
	...
	*/
	...
	ELSE BEGIN
		...
		ELSE BEGIN
			...
			IF @userid is  null
			--	BEGIN
			--		SET @fields = 'username,lastModified,Download_Datetime,CLASSIFICATION,CONTYPE,ARTICLE_TYPE,TITLE,TITLE_SECONDARY
			...
			--		EXEC dbo.shadow_screen_data @webservices_requests_id=@webservices_requests_id,@table='_DM_INTELLCONT'
			...
			--ELSE
				BEGIN
					...
					SET @current_record_auth_count = @tolerance * @current_record_auth_count
						...
						BEGIN
							-- we can copy FROM (#_DM_INTELLCONT, and #_DM_INTELLCONT_AUTH) TO (_DM_INTELLCONT, and _DM_INTELLCONT_AUTH)
							...
							--UPDATE dbo._DM_INTELLCONT 
							--	SET FACSTAFFID = U.FACSTAFFID, EDWPERSID=U.EDWPERSID 
							--FROM dbo._DM_USERS U, dbo._DM_INTELLCONT F
							--WHERE U.userid = F.userid AND ( F.FACSTAFFID is NULL OR F.EDWPERSID IS NULL )			

							TRUNCATE TABLE dbo._DM_INTELLCONT_AUTH
							INSERT INTO dbo._DM_INTELLCONT_AUTH
								(
							...
							INSERT INTO DM_Shadow_Production.dbo._DM_INTELLCONT
								(
									 [id], lastModified, Create_Datetime, Download_Datetime
									 ,CLASSIFICATION,CONTYPE,ARTICLE_TYPE,TITLE,TITLE_SECONDARY
									 ,[STATUS],JOURNAL_REF,JOURNAL_ID,JOURNAL_REFEREED,JOURNAL_NAME,JOURNAL_REVIEW_TYPE
							...
							SELECT  [id], lastModified, Create_Datetime, Download_Datetime
									 ,CLASSIFICATION,CONTYPE,ARTICLE_TYPE,TITLE,TITLE_SECONDARY
									 ,[STATUS],JOURNAL_REF,JOURNAL_ID,JOURNAL_REFEREED,JOURNAL_NAME,JOURNAL_REVIEW_TYPE
									 ,PUBLISHER,PUBCTYST,VOLUME,ISSUE,PAGENUM
									 ,REVISED,INVITED,UNDER_REVIEW,EDITORS
							...
							--SET FACSTAFFID = U.FACSTAFFID, EDWPERSID=U.EDWPERSID 
							--FROM DM_Shadow_Production.dbo._DM_USERS U, DM_Shadow_Production.dbo._DM_INTELLCONT F
							--WHERE U.userid = F.userid AND ( F.FACSTAFFID is NULL OR F.EDWPERSID IS NULL )
