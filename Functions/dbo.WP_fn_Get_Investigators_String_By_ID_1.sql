...
-- NS 11/28/2017: WP_fn_Get_Investigators_String_By_ID_1
--		Grants in _DM_CONGRANT are identified by [ID]
--		that match [ID] in _DM_CONGRANT_INVEST
--		Contrary to [WP_fn_Get_Investigators_String_By_ID_2]
--		This bibliography will not put the Profile URL to the author whose facstaff_ID = @FSID1
--		but will put the Profiel URL to everyone else as long as they are Business employees


CREATE FUNCTION [dbo].[WP_fn_Get_Investigators_String_By_ID_1](@FSID1 INT, @ID BIGINT)
RETURNS VARCHAR(MAX)
AS
BEGIN


	--	PRINT dbo.[WP_fn_Get_Investigators_String_By_ID_1] (10902, '144075059200') -- cxchen, Clara Chen
...
--SELECT dbo.Adhoc_sp_Get_Authors_String_By_FSID_RPID1('1','')
GO
