...
-- NS 11/27/2017: WP_fn_Get_Authors_String_By_ID_2
--		Based on Faculty_Staff_Holder.dbo.WP_fn_Get_Authors_String_By_FSID_RPID1
--		Publications in _DM_INTELLCONT are identified by [ID]
--		that match [ID] in _DM_INTELLCONT_AUTH
-- NS 2/23/2015   (revised on 11/27/2017)
--		This function is used by [WP_sp_Get_Publications_By_Group_And_Type] SP
--		Contrary to [WP_fn_Get_Authors_String_By_ID_1]
--		This bibliography will put the Profile URL to all of the authors as long as they are Business employees

CREATE     FUNCTION [dbo].[WP_fn_Get_Authors_String_By_ID_2](@ID BIGINT)
RETURNS VARCHAR(MAX)
AS
BEGIN

	--PRINT dbo.[WP_fn_Get_Authors_String_By_ID_2] (144216225792)
